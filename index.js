...
function main() {
  ...
  if (configFile.defs) configData.defs = configFile.defs;
  ...
  if (configFile.buildPath) configData.buildPath = configFile.buildPath;
  ...
  console.info(
    chalk.yellowBright("Building into: ") + '"' + configData.buildPath + '"'
  );
  ...
  } else fs.mkdirSync(configData.buildPath);
  ...
  if (myArgs.includes("-w")) {
    ...
    watch(PATH, { recursive: true, delay: 150 }, function (evt, filePath) {
      ...
      if (evt == "update") {
        if (
          path
            .normalize(filePath)
            .split(path.sep)
            .some((r) => configData.ignoreDir.includes(r))
        )
          return;
        ...
        if (path.extname(filePath) == ".php") {
          processFile(
            configData.buildPath +
              path.sep +
              filePath.replace(process.cwd(), "")
          );
        } else {
          console.info(
            chalk.greenBright("Copying file: ") +
              filePath.replace(process.cwd(), "")
          );
        }
      }
...
function copy(pathData) {
  ...
  fse.copySync(
    PATH + "/" + pathData,
    configData.buildPath + path.sep + pathData,
    { overwrite: true },
    (err) => {
      if (err) {
        console.error(err);
...
function processFile(file) {
  ...
  processData
    ...
    .forEach((pd) => {
      if (!pd.elseLine) {
        if (
          configData.defs.includes(
            linesArray[pd.startLine].trim().split(" ")[1]
          ) == pd.ndef
        ) {
